...
<#
    PDF Lecture Concatenator - installer
    ------------------------------------
...
$App = @{
    Name          = 'PDF Lecture Concatenator'
    Blurb         = 'Merge lecture PDFs in the right order, in your browser'
    Repo          = 'https://github.com/semyonsw/pdf-concatinator.git'
    Issues        = 'https://github.com/semyonsw/pdf-concatinator/issues'
    WindowsOnly   = $false

    MinPython     = '3.10'
    Venv          = '.venv'
    Requirements  = 'requirements.txt'
    VerifyImports = @('pypdf', 'fastapi', 'uvicorn', 'multipart', 'tkinter')

    MinNode       = '18.0'
    NpmDirs       = @( @{ Path = 'frontend'; Label = 'browser interface' } )

    EnvKeys       = @()

    ExtraSteps = @(
        @{ Title  = 'Building the browser interface'
           Action = {
               param($PyExe)
               $npm = Get-Npm
               $r = Run -Exe $npm -Arguments @('run', 'build') -WorkDir (Join-Path $Root 'frontend')
               if ($r.Code -eq 0) { OK 'the browser interface was built and checked' }
               else { Warn 'the production build of the interface failed.' 'Not fatal: the launcher starts the development server, which works the same. See install.log.' }
           } },
        @{ Title  = 'Running the project test suite'
           Action = {
               param($PyExe)
               $r = Run -Exe $PyExe -Arguments @('-m', 'pytest', '-q')
               if ($r.Code -eq 0) {
                   ...
                   Warn 'some of the project tests failed.' 'The app will still start. The output is in install.log, at the end.'
               }
           } }
    )

    Launchers = @(
        @{ Name = 'Start PDF Concatenator.bat'; Body = @'
@echo off
rem Written by Install.bat. Starts the merger and opens it in your browser.
...
if not exist "frontend\node_modules" (
    echo.
    echo  [ERROR] The browser interface is not installed ^(no frontend\node_modules^).
...
echo.
echo  Starting the PDF Concatenator...
echo.

if not exist ".web_data" mkdir ".web_data" >nul 2>&1

powershell -NoProfile -Command "$p = Start-Process -PassThru -WindowStyle Minimized -FilePath '.venv\Scripts\python.exe' -ArgumentList '-m','uvicorn','backend.app.main:app','--host','127.0.0.1','--port','8000'; Set-Content -Path '.web_data\engine.pid' -Value $p.Id"

echo  Waiting for the engine to come up...
powershell -NoProfile -Command "for($i=0;$i -lt 60;$i++){ try{ Invoke-WebRequest -UseBasicParsing 'http://127.0.0.1:8000/api/health' -TimeoutSec 2 | Out-Null; exit 0 } catch { Start-Sleep -Milliseconds 500 } }; exit 1"
if errorlevel 1 (
    echo.
    echo  [ERROR] The engine did not start.
    echo.
    echo   * Port 8000 may already be in use. Check with:
    echo         netstat -ano ^| findstr :8000
    echo   * Or run it in the foreground to see the real error:
    echo         .venv\Scripts\python.exe -m uvicorn backend.app.main:app --port 8000
    echo.
    pause
    goto :stop
)

echo  Engine ready. Opening your browser...
echo.
echo    Keep this window open while you work. Closing it stops the app.
echo.
call npm --prefix frontend run dev -- --open

:stop
powershell -NoProfile -Command "if (Test-Path '.web_data\engine.pid') { $id = Get-Content '.web_data\engine.pid'; Stop-Process -Id $id -Force -ErrorAction SilentlyContinue; Remove-Item '.web_data\engine.pid' -Force -ErrorAction SilentlyContinue }"
exit /b 0
'@ },
        @{ Name = 'Start PDF Concatenator (desktop window).bat'; Body = @'
@echo off
rem Written by Install.bat. The older desktop version: one window, no browser.
setlocal EnableExtensions
cd /d "%~dp0"
...
start "" ".venv\Scripts\pythonw.exe" "pdf_concatenator_gui.py"
'@ }
    )

    Shortcut = @{ Name = 'PDF Concatenator'; Target = 'Start PDF Concatenator.bat'; Icon = '' }

    NextSteps = @(
        'Double-click "PDF Concatenator" on your Desktop  (or "Start PDF Concatenator.bat" here).',
        'Your browser opens on http://localhost:5173 - drag your PDFs in, order them, merge.',
        '',
        'Keep the black window open while you work; closing it stops the app.',
        '',
        'Prefer a plain desktop window?  "Start PDF Concatenator (desktop window).bat"'
    )
}
